...
export default function Register() {
  ...
  function submit() {
    if (otp === subotp) {
      ...
      fetch(`${process.env.REACT_APP_SERVER_PREFIX}/insertdata`, { method: 'post', body: JSON.stringify({ name, email }), headers: { 'content-type': 'application/json' } });
      alert('inserted');
    } else {
    ...
    <div className="registerForm">
      <div className="whiteBg">
        ...
        onResolve={({ provider, data }) => {
          ...
          const {name}=data;
          const {email}=data;
          fetch(`${process.env.REACT_APP_SERVER_PREFIX}/googledata`, { method: 'post', body: JSON.stringify({ name, email }), headers: { 'content-type': 'application/json' } });
        }}
        onReject={(err) => {
          console.log(err);
        }}
